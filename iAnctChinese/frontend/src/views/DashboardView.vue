    ...
    <div v-else class="graph-stage">
      <div class="graph-grid">
        ...
        <section class="panel view-panel">
          ...
          <component :is="currentComponent" v-bind="viewProps" @jump-to-text="handleJumpToText" />
        </section>
...
const handleJumpToText = (position) => {
  // 切换到结构标注阶段
  stage.value = "structure";

  // 传递位置信息给 TextWorkspace
  // 可以通过 store 或其他方式传递
  console.log('跳转到文本位置:', position);

  // TODO: 实现具体的跳转逻辑，高亮显示对应文本
  // 将来可以通过 store 或 provide/inject 传递给 TextWorkspace 组件
};
</script>

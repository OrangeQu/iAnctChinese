...
- **DashboardView**：左侧为导航树（项目→类型→文献→章节）与过滤器，中间视图区支持知识图谱、时间轴、地图、战争对抗视图与统计等视角切换。顶部「文本类型推断」可一键触发模型判定/自动标注，下方推荐视图标签引导老师切换。全局搜索框触发对话框列出匹配文献，可直接跳转；导出按钮下载 JSON。
...
3. **更多可视化模板**：人物传记可扩展仕途桑基图；战争类可叠加对抗方颜色区分。
